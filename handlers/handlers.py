...
from find_group import group_search
...
from schedule_parce.get_schedule_object import get_sched
...
async def search_group(message: types.Message, state: FSMContext):
    comp_match, match, other_match = await group_search(message.text)
    if comp_match:
        ...
        await message.answer(text='Что ты ввел? Попробуй еще раз ввести название группы.')
...
async def reg_final(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        print(e)
        await message.answer(text='Произошла ошибка, обратитесь к разработчику. Дань попробуй заново пройти регистрацию)', reply_markup=kb.anon_kb)
        await state.finish()
        await AnonStates.anon.set()
