...
class Database:
    ...
    async def check_lock_group(self, id_chat: int, group_id: int) -> str:
        sql_query = "select lock from groups_students where id = $1"
        logger.info(f'Student - {id_chat} check lock group')
    ...
    async def check_user(self, id_chat: int) -> str:
        ...
        logger.info(f'Student - {id_chat} check_user')
    ...
    async def check_tester(self, id_chat: int):
        ...
        logger.info(f'Student - {id_chat} check_tester')
        return await self.pool.fetch(sql_query, id_chat)
